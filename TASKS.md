...
- [x] Inspected the repository: Prisma `Product` schema (no `ProductVariant` model — `Product` is the purchasable entity), admin product create/edit (`product-form.tsx`, `actions.ts`, `lib/validations/product.ts`, `lib/db/products.ts`), cart → checkout line loader (`lib/db/cart.ts` `getCartProduct`, `features/checkout/checkout-lines.ts`), shipping quote request building (`features/shipping/shipping-logic.ts` `buildQuoteRequest`/`parcelsFromItemCount`), the `ShippingProvider` abstraction, the Bob Go adapter (`features/shipping/couriers/bobgo.ts` `bobGoParcels` per-field fallback), `lib/db/payments.ts` `createCheckoutIntent`, and the existing tests. GUIDEBOOK.md untouched.
- [x] Additive Prisma schema change (no data reset, no existing product modified): `Product.weightGrams`, `Product.lengthCm`, `Product.widthCm`, `Product.heightCm` — all `Int?` (nullable). Integer grams/centimetres keep the schema consistent with the existing `Int`-based inventory fields. Existing products remain valid (all four nullable). Fields added to `Product` (the purchasable entity) — no duplicate fields on other models.
- [x] Additive Prisma migration created and applied: `prisma/migrations/20260903000000_add_product_weight_dimensions/migration.sql` (pure `ALTER TABLE "products" ADD COLUMN ... INTEGER` — additive only). Applied via `prisma db execute` (no reset) and marked applied via `prisma migrate resolve`. `prisma migrate diff` confirms the DB is in sync with the schema (no drift).
- [x] Validation (`lib/validations/product.ts`): four new optional Zod fields — non-negative integer strings (`/^\d+$/`). Negative values rejected; decimal/non-numeric values rejected; zero accepted (treated as "not specified" by the shipping layer); null/empty/omitted accepted (truly optional). Decision documented: zero is permitted by validation but treated as missing by the shipping fallback because a zero physical measurement is not a meaningful shipping input (Bob Go requires dimensions ≥ 1 cm and a positive weight).
- [x] Admin product management: `ProductInput`/`ProductUpdateInput` extended; `createProduct`/`updateProduct` persist the four fields (nullable); `createProductAction`/`updateProductAction` parse them via a shared `parseOptionalInt` helper (empty → null) and `productFormValues` collector; `ProductForm` UI adds a "Shipping dimensions" section (Weight grams, Length/Width/Height cm) with `min="0"` number inputs, clearly labelled optional, defaulting from the existing product's values on edit. Follows the existing RHF + Zod + shadcn/ui form conventions.
- [x] Checkout/shipping data flow: `getCartProduct` (`lib/db/cart.ts`) and `AuthoritativeCheckoutLine` (`features/checkout/checkout-lines.ts`) now carry the four physical fields from PostgreSQL. `buildQuoteRequest` accepts an optional `lines` array of per-line physical data; new `parcelsFromLines` builds one parcel per purchased unit carrying the product's actual weight/dimensions. `getCheckoutSummaryAction`, `getShippingQuoteAction`, and `createCheckoutIntent` (`lib/db/payments.ts`) all pass `lines` so the authoritative quote at intent time uses the same physical data as the displayed summary.
- [x] Quantity handling: purchasing quantity Q of a product emits Q parcels each carrying that product's own weight/dimensions — NOT a single parcel with multiplied dimensions (multiplying dimensions would be physically incorrect). This matches the existing `parcelsFromItemCount` convention (one parcel per item) and the Bob Go `parcels` array contract.
- [x] Fallback behavior (deterministic, backward compatible): weight — missing/null/zero → `DEFAULT_PARCEL_WEIGHT_GRAMS` (1 kg) applied by `parcelsFromLines`; dimensions — missing/null/zero → omitted (`undefined`) on the parcel, then `bobGoParcels` applies `DEFAULT_PARCEL_*_CM` (30×20×10 cm) per-field (Bob Go requires dimensions ≥ 1 cm). Per-field fallback means partial data (e.g. weight but no dimensions) keeps the supplied field and falls back per-field for the missing ones. Products with NO physical metadata produce parcels identical to the pre-existing `parcelsFromItemCount` behaviour, so existing products continue to receive quotes exactly as before.
- [x] Bob Go adapter: `bobGoParcels` already mapped per-field (`parcel.lengthCm ?? DEFAULT_PARCEL_*_CM`), so no logic change was required — only the LIMITATION documentation was updated to reflect that real product dimensions/weight now flow through. Response parsing and quote normalization are unchanged (dimension-agnostic).
- [x] Tests: 40 new tests — `lib/validations/product.test.ts` (20: valid/omitted/null/empty/zero/partial accepted; negatives/decimals/non-numeric/leading-sign rejected; existing required-field enforcement unchanged); `features/shipping/shipping-logic.test.ts` (12 new: `parcelsFromLines` quantity → one parcel per unit, no dimension multiplication, multi-line combination, per-field fallback for missing/null/zero weight & dimensions, partial data, no-data matches legacy behaviour, empty/degenerate guard, fractional quantity flooring; `buildQuoteRequest` uses `lines` when supplied and falls back to `parcelsFromItemCount` when omitted); `features/shipping/couriers/bobgo.test.ts` (8 new: actual weight/dimensions used in `bobGoParcels`, per-field default fallback for missing/partial dimensions, `quote()` sends actual dimensions/weight in the live request body, quantity > 1 → one parcel per unit with no dimension multiplication, fallback dimensions/weight for products without physical data, zero treated as missing, multi-line combination, response parsing/normalization unchanged with dimensions). Total suite: 341 tests.
- [x] Existing functionality unchanged: Redis cart, payment providers, checkout, account, admin, search, shipping abstraction/logic — no unrelated code modified. Existing 301 tests kept passing.
- [x] Documentation: PROJECT.md, TASKS.md, README.md updated; GUIDEBOOK.md untouched.
- [x] Verification: `npx prisma validate` (valid), `npx prisma generate`, `npx tsc --noEmit` (clean), `npm test` (341 tests, 0 fail), `npx eslint .` (0 errors, 4 pre-existing `_query`-unused warnings), `npx next build` (success), `npx prisma migrate diff` (no drift — DB in sync with schema). Migration present and consistent.
- [x] **Live sandbox verification (Post-MVP #5 live):** `BOBGO_API_KEY` + `BOBGO_TEST_MODE=true` present in `.env.local`. A single real, non-destructive `POST /rates` was performed against the Bob Go sandbox (`https://api.sandbox.bobgo.co.za/v2`) using the existing `BobGoProvider` (no redesign) with a parcel carrying known physical dimensions: 1200 g, 35×22×12 cm, quantity 2 → 2 parcels (Cape Town → Sandton, ZAR 200 subtotal). The live request body was captured and verified to carry the actual measurements (`submitted_weight_kg: 1.2`, `submitted_length_cm: 35`, `submitted_width_cm: 22`, `submitted_height_cm: 12`, 2 parcels — no dimension multiplication). The sandbox returned HTTP 200 with a real ZAR rate (`155.85 ZAR` via "Economy", `provider: "bobgo"`, `estimateDays: null`) that normalized correctly. No shipments, labels, collections, orders, or payments were created (rate request only). The API key was never printed/exposed/committed. Status: **live-sandbox-verified with real product dimensions**; NOT production-verified (the production base URL has not been exercised).
