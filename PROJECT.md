...
- **Tests:** 28 search-logic unit tests (`features/search/search-logic.test.ts`) covering name/description/sku/category matching, case-insensitive matching, partial/substring matches, no results, archived-product exclusion, empty/invalid/over-long query handling, result ordering (name ascending), and limit clamping. Total test suite: 111 tests.
