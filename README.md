...
- **Stitch** — **pending**. The Svix/Standard Webhooks signature verification (`svix-*` headers, `STITCH_WEBHOOK_SECRET`) and the OAuth 2.0 client-credentials token helper are implemented and tested, but the hosted-payment-session creation flow (the `clientPaymentInitiationRequestCreate` GraphQL mutation) is not — its exact input contract is not available in a form that can be implemented accurately, and the guidebook forbids inventing API request/response shapes. `StitchProvider.isConfigured()` returns false until that mutation is wired up, so Stitch can never be selected by checkout.
