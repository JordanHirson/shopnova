...
- **Live courier status:** **Bob Go** (the guidebook's multi-courier API) is implemented and registered (`features/shipping/couriers/bobgo.ts`) — it is selected only when `BOBGO_API_KEY` is present. It uses the official public REST API (api-docs.bob.co.za): Bearer-token auth, `POST /rates`, sandbox (`https://api.sandbox.bobgo.co.za/v2`) via `BOBGO_TEST_MODE=true` and production (`https://api.bobgo.co.za/v2`) otherwise. It parses every documented response shape (a bare array, `{rates|data|results}`, `{provider_rate_requests[].responses[]}`) and maps each rate to a `ShippingQuote`. Implemented per the official public API contract and unit-tested with mocked `fetch`; **not** exercised against the live Bob Go sandbox (no `BOBGO_API_KEY` in this environment), so it is not marked production-verified. Aramex, PUDO, and The Courier Guy adapters remain pending official API contracts/credentials.
- **Per-product weight/dimensions limitation:** the MVP product schema does NOT carry per-variant weight or dimensions, so a Bob Go live quote is an APPROXIMATION based on the existing documented default weight (1 kg) and a conservative default parcel size (30×20×10 cm) — NOT a verified accurate charge for the actual goods. This is documented prominently in the adapter; accurate quotes require the post-MVP per-product weight/dimensions schema addition. No weights or dimensions are invented silently.
